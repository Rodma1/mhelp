  ...
  <div class="publishTost">
    <div class="content">
      ...
      <div class="cancle" @click="cancle">
        <img src="@/assets/img/example/cancel.png" alt="" />
      </div>
      <div class="item discribe">
        <div>备注:</div>
        <textarea placeholder="可为空" v-model="params.remark"></textarea>
      </div>
      <div class="item">
        <div>分类:</div>
  ...
  methods: {
    ...
    publish() {
      ...
      if (this.params.category && this.params.tags && this.params.price) {
        ...
        console.log(this.params.price < this.userInfo.money)
        if (this.params.price < this.userInfo.money||this.params.price ==this.userInfo.money) {
          this.$emit("publish");
        }
        else{
          this.$toast("金币不足，无法发布任务")
        }
      } else {
